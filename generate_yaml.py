...
while count <= 200:
    for res in resources:
        ...
        output_data.append({
            "title": f'{res["title"]}{title_suffix}',
            "difficulty": diff,
            "topic": res["topic"],
            "type": res["type"],
            "desc": res["desc"],
...
with open('docs/_data/scale_test.yml', 'w') as f:
    for item in output_data:
        f.write(f"- title: {json.dumps(item['title'])}\n")
        f.write(f"  difficulty: {json.dumps(item['difficulty'])}\n")
        f.write(f"  topic: {json.dumps(item['topic'])}\n")
        f.write(f"  type: {json.dumps(item['type'])}\n")
        f.write(f"  desc: {json.dumps(item['desc'])}\n")
        f.write(f"  link: {json.dumps(item['link'])}\n")
        f.write(f"  link_text: {json.dumps(item['link_text'])}\n")
